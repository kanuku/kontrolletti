...
class SynchronizerTest extends FlatSpec with MockitoSugar with MockitoUtils {
  ...
  private val synchronizer = new SynchronizerImpl(oAuthClient, store, kioClient, search)
  ...
  "Synchronizer#synchCommits" should "store apps from kio in data-store" in {
    val scmUrls = List("https://github.com/zalando/kontrolletti/", "https://git-hub.com/zalando-bus/lizzy/")
    ...
    when(store.scmUrls()).thenReturn(scmUrls)
